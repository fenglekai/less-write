...
function limitBrink(limitX: number, limitY: number) {
  ...
  // // 计算底图与底图高度差距
  const bottomLimit = HEIGHT - imgRenderHeight * BASE_SCALE ** scaleCount.value;
  if (limitY < bottomLimit) {
    // 当底图实际宽度小于地图宽度设为0
    limitY = bottomLimit > 0 ? 0 : bottomLimit;
  }
  const rightLimit = WIDTH.value - WIDTH.value * BASE_SCALE ** scaleCount.value;
...
export function init(params: GroupParams) {
